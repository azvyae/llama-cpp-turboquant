...
// CI builds first then runs these tests; local devs should run `bun run build` or use `bun run test:pwa`.
describe("PWA Build Output", () => {
	if (!distExists) {
		console.warn(
			`⚠ Skipping PWA Build Output tests - dist/ not found (run 'bun run build' first)`,
		);
	...
	describe("Core files exist", () => {
		it("service worker (sw.js) exists", () => {
			expect(
				existsSync(resolve(DIST_DIR, "sw.js")),
				"sw.js not found",
			).toBeTruthy();
		});
		...
		it("SvelteKit bundle.css exists in _app/immutable/assets/", () => {
			...
			expect(
				existsSync(cssDir),
				"_app/immutable/assets/ not found",
			).toBeTruthy();
	...
	describe("version.json content", () => {
		it("has valid JSON with version field", () => {
			const content = readFileSync(
				resolve(DIST_DIR, "_app", "version.json"),
				"utf-8",
			);
	...
	describe("Service worker content", () => {
		...
		it("precache contains SvelteKit bundle.js with content hash", () => {
			...
			expect(swContent).toMatch(
				/"_app\/immutable\/bundle\.[a-zA-Z0-9_-]+\.js"/,
			);
		});
		...
		it("precache contains SvelteKit bundle.css with content hash", () => {
			...
			expect(swContent).toMatch(
				/"_app\/immutable\/assets\/bundle\.[a-zA-Z0-9_-]+\.css"/,
			);
		});
	...
	describe("index.html content", () => {
		it("has modulepreload link for SvelteKit bundle with content hash", () => {
			...
			expect(indexContent).toMatch(
				/href="(\.\/|\/)_app\/immutable\/bundle\.[a-zA-Z0-9_-]+\.js"/,
			);
		});
