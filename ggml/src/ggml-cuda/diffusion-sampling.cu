...
static __global__ void diffusion_dense_sample_kernel(const float * __restrict__ logits,
...
bool ggml_cuda_diffusion_sample(struct ggml_tensor * logits,
                                const float *        u_host,
                                int *                argmax_host,
                                float *              entropy_host,
                                int *                sampled_host,
    ...
    diffusion_dense_sample_kernel<<<n_tokens, 256, 0, 0>>>(logits_d, s.u, s.argmax, s.entropy, s.sampled, n_vocab,
                                                           inv_temp);
