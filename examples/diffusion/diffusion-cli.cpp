...
static bool diffusion_step_callback(int32_t             step,
    ...
    auto print_progress_bar = [](int32_t step, int32_t total_steps) {
        ...
        LOG_INF("\rdiffusion step: %d/%d [%s%s] %d%%", step, total_steps, std::string(progress_bars, '=').c_str(),
                std::string(50 - progress_bars, ' ').c_str(), progress_percent);
    };
    ...
    if (data->diff_params->visual_mode) {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            if ((int) ln.size() > cols) {
                ln.resize(cols);
            }  // clamp width so the row never wraps
            frame += ln + "\033[K";
            if (r < rows - 1) {
                frame += "\n";
            }
...
int main(int argc, char ** argv) {
    ...
    if (canvas_length > 0 && params.n_predict > 0) {
        ...
        LOG_INF("diffusion: -n %d -> %d blocks, n_ubatch=%d n_batch=%d n_ctx=%d (canvas_length=%d)\n", params.n_predict,
                blocks, params.n_ubatch, params.n_batch, params.n_ctx, cl);
    }
    ...
    if (params.fit_params) {
        LOG_INF(
            "diffusion: --fit has no effect here; context is sized from -n and the canvas. "
    ...
    callback_data cb_data        = {
        &diff_params, vocab, 0, params.diffusion.visual_progress, std::max(1, params.diffusion.visual_interval), 0, 0,
        24,           80,    0
    };
    ...
    LOG_INF("diffusion_params: steps=%d schedule=%d algorithm=%d temperature=%.3f eps=%.6f mask_token=%d\n",
            diff_params.steps, (int) diff_params.schedule, (int) diff_params.algorithm, diff_params.temperature,
            diff_params.eps, mask_token_id);
    ...
    if (use_eb) {
        ...
        auto meta_i = [&](const char * key, int32_t def) -> int32_t {
            char buf[32];
            return llama_model_meta_val_str(model, key, buf, sizeof(buf)) >= 0 ? (int32_t) strtol(buf, nullptr, 10) :
                                                                                 def;
        };
        ...
        if (params.diffusion.eb_t_min >= 0) {
            eb_params.t_min = params.diffusion.eb_t_min;
        }
        if (params.diffusion.eb_t_max >= 0) {
            eb_params.t_max = params.diffusion.eb_t_max;
        }
        if (params.diffusion.eb_entropy_bound >= 0) {
            eb_params.entropy_bound = params.diffusion.eb_entropy_bound;
        }
        if (params.diffusion.eb_stability >= 0) {
            eb_params.stability_threshold = params.diffusion.eb_stability;
        }
        if (params.diffusion.eb_confidence >= 0) {
            eb_params.confidence_threshold = params.diffusion.eb_confidence;
        }
        if (params.diffusion.eb_max_steps > 0) {
            eb_params.max_denoising_steps = params.diffusion.eb_max_steps;
        }
        ...
        LOG_INF(
            "diffusion_eb: max_steps=%d t=[%.3f,%.3f] entropy_bound=%.4f stability=%d confidence=%.4f kv_cache=%s "
            "gpu_sampling=%s sample_reduce=%s\n",
    ...
    auto run_turn = [&](const std::string & formatted_prompt) -> std::string {
        ...
        if (canvas_length <= 0) {
            ...
            return common_detokenize(
                vocab, std::vector<llama_token>(output_tokens.begin() + n_input, output_tokens.begin() + n_generated),
                false);
        }
        ...
        for (int b = 0; b < n_blocks; b++) {
            ...
            if (max_length > max_ub) {
                if (b == 0) {
                    LOG_ERR(
                        "error: this diffusion model needs the whole [prompt | canvas] in one ubatch; "
            ...
            int32_t n_generated = 0;
            if (use_eb) {
                diffusion_generate_entropy_bound(ctx, prefix.data(), output_tokens.data(), prefix_len, eb_params,
                                                 n_generated);
            } else {
