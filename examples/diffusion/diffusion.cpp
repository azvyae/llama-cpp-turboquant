...
void diffusion_generate(llama_context *          ctx,
    ...
    for (int block_num = 0; block_num < num_blocks; block_num++) {
        int32_t block_start = (params.schedule == DIFFUSION_TRANSFER_SCHEDULE_BLOCK_BASED) ?
                                  n_input + block_num * params.block_length :
                                  0;
        ...
        for (int32_t step = 0; step < steps_per_block; step++) {
            ...
            if (params.step_callback) {
                if (!params.step_callback(global_step, params.steps, output_tokens, params.max_length,
            ...
            if (params.algorithm == DIFFUSION_ALGORITHM_ORIGIN) {
                int32_t transfer_count = calculate_transfer_count(step, steps_per_block, mask_positions.size(),
                                                                  params.schedule, params.eps, num_transfer_tokens);
...
void diffusion_generate_entropy_bound(llama_context *             ctx,
    ...
    for (int32_t cur_step = S; cur_step >= 1 && !finished; --cur_step) {
        ...
        llama_diffusion_set_sc(model, dev_sc ? nullptr : sc_buffer.data(), step_idx == 0 ? 0.0f : 1.0f, prev_temp_inv,
                               true);
        ...
        auto run_host_worker = [&]() {
            ...
            for (unsigned ti = 0; ti < nth; ti++) {
                ...
                if (p0 < p1) {
                    pool.emplace_back(worker, p0, p1);
                }
            }
            for (auto & th : pool) {
                th.join();
        ...
        if (gpu_reduce) {
            ...
            if (!llama_diffusion_device_sample(model, u.data(), argmax_canvas.data(), entropy.data(), denoiser.data(),
                                               C, temp_inv)) {
